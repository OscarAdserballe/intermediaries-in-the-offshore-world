...
# %%
# Get all intermediary-officer relationships
intermediary_connections = relationships_df[relationships_df['rel_type'] == 'intermediary_of']

# Count number of officers per intermediary
officers_per_intermediary = intermediary_connections.groupby('node_id_start').size()

# Create distribution plot
plt.figure(figsize=(12, 6))
sns.histplot(data=officers_per_intermediary, log_scale=True, bins=50)
plt.title('Distribution of Officers per Intermediary')
plt.xlabel('Number of Officers (log scale)')
plt.ylabel('Count of Intermediaries')

# Add median and mean annotations
median = officers_per_intermediary.median()
mean = officers_per_intermediary.mean()
plt.axvline(median, color='red', linestyle='--', alpha=0.5, label=f'Median: {median:.1f}')
plt.axvline(mean, color='green', linestyle='--', alpha=0.5, label=f'Mean: {mean:.1f}')
plt.legend()

plt.show()

# Print some basic statistics
print(f"Number of intermediaries with at least one officer: {len(officers_per_intermediary)}")
print(f"Average number of officers per intermediary: {mean:.1f}")
print(f"Median number of officers per intermediary: {median:.1f}")
print(f"Max number of officers for a single intermediary: {officers_per_intermediary.max()}")
...
plt.title('Distribution of Officers per Intermediary with Power Law Fit')
plt.xlabel('Number of Officers (log scale)')
...
class SearchLogEntry(TypedDict):
    ...
    iteration: int
    query: str
    results: List[Dict[str, Any]] # Store raw results from Tavily
    error: Optional[str] # Store potential search errors

class State(TypedDict):
    """The workflow state."""
    intermediary_data: IntermediaryData # Basic info (name)
    search_log: List[SearchLogEntry]   # Log of all searches performed
    search_iteration: int              # Current search iteration count
    max_iterations: int                # Maximum allowed search iterations
    current_query: Optional[str]       # The query to be executed in the *next* search step
    needs_more_info: bool              # Flag set by analysis node if more search is needed
    classification_result: Optional[ClassificationResult] # Final result
    error: Optional[str]               # For capturing general workflow errors
...
llm = ChatGoogleGenerativeAI(
    ...
    temperature=0,
    max_tokens=None, # Let the model decide or set appropriately
    timeout=None,
    max_retries=2,
)
...
def initial_search_node(state: State) -> Dict[str, Any]:
    ...
    query = intermediary_name # Initial query is just the name
    search_results = []
    search_error = None
...
def additional_search_node(state: State) -> Dict[str, Any]:
    ...
    if not query:
        ...
        # It's possible the graph transitions here even if no query was set, though ideally analyze_node prevents this.
        # We avoid adding an empty log entry.
        return {} # No changes to state if no query
...
def analyze_and_decide_node(state: State) -> Dict[str, Any]:
    ...
    """
    iteration = state["search_iteration"] # Represents the iteration *about* to happen or the end
    intermediary_name = state["intermediary_data"].name
    ...
    # Prepare context from search log for the decision model
    # Let's provide a summary or just the latest results to avoid overly long prompts
    ...
    # Or provide more detail if needed: json.dumps(search_log, indent=2)
    ...
    if search_decision.needs_more_info and iteration < max_iterations and search_decision.additional_query_string:
        ...
        return {
            ...
            # search_iteration is already incremented by the search node, analysis prepares for the *next* state
        }
    else:
        ...
        else: # Needs more info BUT no query provided or max iterations reached
        ...
        # Prepare context from the full search log for the classification model
        log_summary_for_classification = json.dumps(search_log, indent=2) # Provide full log here
...
def create_workflow(max_iterations: int = MAX_ITERATIONS):
    ...
    # Conditional edge: After analysis, either search more or end
    workflow.add_conditional_edges(
        "analyze_and_decide",
        # The lambda function checks the 'needs_more_info' flag set by analyze_and_decide_node
    ...
    # Loop: After additional search, go back to analysis
...
def classify_intermediary(intermediary_name: str, max_iterations: int = 3) -> Dict[str, Any]:
    ...
    final_state = None
    try:
        # Configuration for debugging/visualization can be added here if needed
        # e.g., config={"recursion_limit": 10}
        ...
        if final_state:
             # Convert ClassificationResult to dict for JSON serialization if needed
        ...
        error_state = final_state or initial_state # Use last known state if available
